...
declare providedTag="oryxdevms"

if [ -n $3 ];then
    # default value 'oryxdevms' for retagging will be used
    providedTag=$3     
fi
echo $providedTag
...
while read buildImage; do
  ...
  if [[ $buildImage != *:latest ]]; then
	...
	newtag=$(echo "$newtag" | sed 's,oryxdevmcr.azurecr.io/public/oryx,'$providedTag',g')
